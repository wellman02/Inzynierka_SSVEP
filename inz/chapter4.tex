...
przygotowania uczestnika, montażu elektrod, sesji zapoznawczej,
dwóch zadań właściwych oraz wypełnienia ankiety. Elektrody EEG umieszczano
w punktach O1, O2 i Oz zgodnie z systemem 10-20, a impedancja
była weryfikowana przed każdą sesją.

Rejestrację wyników zadania skuteczności komend prowadzono na podstawie
logu terminala generowanego przez moduł akwizycji. Każda wykryta
i wykonana komenda była zapisywana w czasie rzeczywistym ze znacznikiem
częstotliwości i wartością współczynnika korelacji kanonicznej $\rho$.
Czasy ukończenia zadania z klikaniem rejestrowano ręcznie przy użyciu
stopera. Po zakończeniu sesji każdy uczestnik wypełniał ankietę
(Załącznik~\ref{appendix:ankieta}).

% -------------------------------------------------------
\subsection*{Zadanie 1 - sterowanie kursorem}
...
na stymulatorach przy jednoczesnej obecności wszystkich pięciu bodźców
...
% -------------------------------------------------------
\subsection*{Zadanie 2 - skuteczność klasyfikacji komend}
...
podczas zadania (skala 1-5) oraz pytań otwartych.
Wyniki liczbowe zestawiono w tabeli~\ref{tab:ankieta}.
...
\subsection*{Przypadek Osoby 3 - zjawisko BCI inefficiency}
...
\subsection*{Problem wielokrotnych aktywacji}
...
\subsection*{Zależność skuteczności od częstotliwości stymulacji}
